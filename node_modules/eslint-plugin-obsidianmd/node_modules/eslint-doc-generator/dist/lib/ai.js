...
const AI_PROVIDERS = Object.values(AI_PROVIDER);
export const SUPPORTED_API_KEY_ENV_VARS = [
    ...new Set(AI_PROVIDERS.map((provider) => PROVIDER_METADATA[provider].apiKeyEnvVar)),
];
...
export function resolveAiProviderConfig({ aiProvider, aiModel, }) {
    if (aiProvider) {
        const metadata = PROVIDER_METADATA[aiProvider];
        const apiKey = getOptionalEnvVar(metadata.apiKeyEnvVar);
        if (!apiKey) {
            throw new Error(`Provider "${aiProvider}" requires ${metadata.apiKeyEnvVar} to be set.`);
        }
        return {
            provider: aiProvider,
            apiKey,
    ...
    const providersWithApiKey = AI_PROVIDERS.flatMap((provider) => {
        const apiKey = getOptionalEnvVar(PROVIDER_METADATA[provider].apiKeyEnvVar);
        return apiKey ? [{ provider, apiKey }] : [];
    });
    if (providersWithApiKey.length === 0) {
        throw new Error(`No AI provider API key found. Set one of: ${SUPPORTED_API_KEY_ENV_VARS.join(', ')}.`);
    }
    if (providersWithApiKey.length > 1) {
        throw new Error(`Multiple AI provider API keys found (${providersWithApiKey
            .map(({ provider }) => PROVIDER_METADATA[provider].apiKeyEnvVar)
            .join(', ')}). Use --ai-provider to specify one.`);
    }
    const { provider, apiKey } = providersWithApiKey[0];
...
function getOpenAiContent(payload) {
    ...
    const [firstChoice] = choicesUnknown;
...
function getAnthropicContent(payload) {
    if (!isRecord(payload)) {
        return undefined;
    }
...
function getProviderErrorDetails(detailsUnknown) {
    ...
    const code = typeof codeUnknown === 'string'
        ? codeUnknown
        : typeof codeUnknown === 'number'
            ? String(codeUnknown)
            : undefined;
    const type = typeof typeUnknown === 'string' ? typeUnknown : undefined;
    const message = typeof messageUnknown === 'string' ? messageUnknown : undefined;
...
function getProtocolErrorDetails(payload) {
    if (!isRecord(payload)) {
        return undefined;
    }
    return (getProviderErrorDetails(payload['error']) ??
        getProviderErrorDetails(payload));
}
function formatProviderErrorDetails(details) {
    if (!details) {
        return undefined;
    }
    const segments = [];
...
async function getHttpErrorDetails(response) {
    let payload;
    try {
...
async function throwProviderHttpError(providerConfig, response) {
...
function parseLlmResponseObject(content) {
    ...
    const jsonLike = firstBrace !== -1 && lastBrace > firstBrace
...
function buildOpenAiCompatibleRequest(providerConfig, prompt) {
    const responseFormatType = providerConfig.provider === AI_PROVIDER.VERCEL_AI_GATEWAY
        ? 'json'
        : 'json_object';
    const requestBody = {
...
function buildAnthropicRequest(providerConfig, prompt) {
    const requestBody = {
        model: providerConfig.model,
        max_tokens: 512,
        temperature: 0,
...
function buildProtocolRequest(providerConfig, prompt) {
...
async function requestProviderObject(providerConfig, prompt) {
    const providerLabel = getProviderLabel(providerConfig.provider);
    const { headers, body, getContent } = buildProtocolRequest(providerConfig, prompt);
    ...
    try {
        ...
        if (abortController.signal.aborted || isAbortError) {
            throw new Error(`${providerLabel} request failed: timed out after ${String(REQUEST_TIMEOUT_MS)}ms.`, { cause: error });
        }
    ...
    if (content === undefined) {
        throw new Error(`${providerLabel} response did not include assistant text content.`);
    }
    return parseLlmResponseObject(content);
}
export function requestAiJsonObject(providerConfig, prompt) {
