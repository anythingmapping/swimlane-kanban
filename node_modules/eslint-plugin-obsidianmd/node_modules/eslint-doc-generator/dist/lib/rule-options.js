...
function hasObjectPath(object, path) {
    return ((Array.isArray(object) ||
        (typeof object === 'object' && object !== null)) &&
        hasProperty(object, path));
}
function getCurrentOptionPath(parentPath, parentKeyword, keyIndex) {
...
function collectNamedOptionsFromSchemaProperties(js, baseOptionPath, metaDefaultOptionForSchema, options) {
    for (const [key, value] of Object.entries(js.properties ?? {})) {
        const type = value.type === 'array' && !Array.isArray(value.items) && value.items?.type
        ...
        const deprecated = 'deprecated' in value ? Boolean(value['deprecated']) : false;
        // Prefer `meta.defaultOptions` over schema `default`.
        let defaultValue = value.default;
        const optionPath = [...baseOptionPath, key];
 ...
 */
export function getAllNamedOptions(jsonSchema, metaDefaultOptions) {
    if (!jsonSchema) {
        return [];
    }
    if (Array.isArray(jsonSchema)) {
        const metaDefaultOptionsForItems = Array.isArray(metaDefaultOptions)
            ? metaDefaultOptions
            : undefined;
        return jsonSchema.flatMap((js, index) => getAllNamedOptions(js, metaDefaultOptionsForItems?.[index]));
    }
    const options = [];
    const optionPathBySchema = new WeakMap([[jsonSchema, []]]);
    traverse(
    // Cast needed because json-schema-traverse types don't account for exactOptionalPropertyTypes
    jsonSchema, (js, _jsonPtr, _rootSchema, _parentJsonPtr, parentKeyword, parentSchema, keyIndex) => {
        const parentPath = parentSchema && optionPathBySchema.has(parentSchema)
            ? optionPathBySchema.get(parentSchema)
            : [];
        const currentPath = getCurrentOptionPath(parentPath, parentKeyword, keyIndex);
        optionPathBySchema.set(js, currentPath);
        if (js.properties) {
            collectNamedOptionsFromSchemaProperties(js, currentPath, metaDefaultOptions, options);
        }
    });
    return options;
}
