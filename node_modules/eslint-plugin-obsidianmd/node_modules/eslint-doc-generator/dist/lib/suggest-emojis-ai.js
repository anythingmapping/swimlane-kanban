...
import { requestAiJsonObject, resolveAiProviderConfig, } from './ai.js';
function normalizeEmojiCandidate(candidate) {
...
function applyAiSuggestions(configNames, llmSuggestions, emojiByConfig) {
    const configNameLookup = new Map(configNames.map((configName) => [configName.toLowerCase(), configName]));
...
export async function applyAiEmojiSuggestions(configNamesToEnhance, emojiByConfig, aiRequestOptions) {
    ...
    const suggestions = await requestAiJsonObject(providerConfig, {
        systemPrompt: 'Return a JSON object mapping each config name to exactly one emoji character. No markdown.',
