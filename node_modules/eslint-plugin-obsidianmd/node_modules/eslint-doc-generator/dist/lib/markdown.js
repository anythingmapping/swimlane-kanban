 ...
 */
export function replaceOrCreateHeader(context, markdown, newHeader, marker) {
    ...
    const postHeader = lines
        ...
        .join(endOfLine);
    return `${preHeader ? `${preHeader}${endOfLine}` : ''}${newHeader}${endOfLine}${postHeader}`;
}
/**
 * Find the section most likely to be the top-level section for a given string.
 */
export function findSectionHeader(context, markdown, str) {
    const { endOfLine } = context;
    ...
    return sectionPotentialMatches.toSorted((a, b) => a.length - b.length)[0];
}
export function findFinalHeaderLevel(context, str) {
    const { endOfLine } = context;
...
export function expectSectionHeaderOrFail(context, contentName, contents, possibleHeaders, expected) {
    const found = possibleHeaders.some((header) => findSectionHeader(context, contents, header));
    if (found !== expected) {
