...
export async function run(argv, cb) {
    ...
    await program
        ...
        .option('--ai [boolean]', `(optional) Whether to use AI for AI-enabled features. (default: ${String(OPTION_DEFAULTS[OPTION_TYPE.AI])})`, parseBoolean)
        .addOption(new Option('--ai-model <model>', '(optional) AI model to use for AI-enabled features. Defaults to the selected provider default model.'))
        .addOption(new Option('--ai-provider <provider>', '(optional) AI provider to use for AI-enabled features. Required if multiple provider API keys are present in the environment.').choices(Object.values(AI_PROVIDER)))
        ...
        .option('--suggest-emojis [boolean]', `(optional) Whether to suggest emojis for configs and print them in a table. Can be paired with \`--ai\`. (default: ${String(OPTION_DEFAULTS[OPTION_TYPE.SUGGEST_EMOJIS])})`, parseBoolean)
