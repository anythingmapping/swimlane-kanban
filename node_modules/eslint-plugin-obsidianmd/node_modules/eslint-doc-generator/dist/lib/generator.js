...
export async function generate(path, userOptions) {
    ...
    // the "context" object.
    const { check, ignoreDeprecatedRules, initRuleDocs, suggestEmojis, pathRuleDoc, pathRuleList, postprocess, ruleDocSectionExclude, ruleDocSectionInclude, ruleDocSectionOptions, } = options;
    if (suggestEmojis) {
    ...
    for (const [name, rule] of ruleNamesAndRules) {
        ...
        if (ruleDocSectionOptions) {
            ...
            for (const { name: namedOption } of getAllNamedOptions(schema, metaDefaultOptions)) {
