...
import { getLinkToRule, getMarkdownLink, replaceRulePlaceholder, } from './rule-link.js';
...
function configsToNoticeSentence(context, configs, severity, configsLinkOrWord, configLinkOrWord) {
    // Create CSV list of configs with their emojis.
    const csv = configs
        .map((config) => {
        const emoji = findConfigEmoji(context, config);
        return `${emoji ? `${emoji} ` : ''}\`${configNameToDisplay(context, config)}\``;
    })
        .join(', ');
 ...
 */
function replacedByToNoticeSentence(context, deprecated, pathCurrentPage) {
    if (!deprecated.replacedBy || deprecated.replacedBy.length === 0) {
        return undefined;
    }
    function formatReplacedByEntry(info) {
        if (!info.rule?.name) {
            return undefined;
        }
        ...
        const externalPlugin = info.plugin?.name && info.plugin.name !== 'eslint'
            ...
            : '';
        return `${replacementRule}${externalPlugin}${info.message ? ` (${info.message})` : ''}${info.url ? ` (${getMarkdownLink('read more', false, info.url)})` : ''}`;
    }
...
const RULE_NOTICES = {
    ...
    [NOTICE_TYPE.CONFIGS]: ({ context, configsError, configsWarn, configsOff, }) => {
        ...
        const sentences = [
            configsToNoticeSentence(context, configsError, SEVERITY_TYPE.error, configsLinkOrWord, configLinkOrWord),
            configsToNoticeSentence(context, configsWarn, SEVERITY_TYPE.warn, configsLinkOrWord, configLinkOrWord),
            configsToNoticeSentence(context, configsOff, SEVERITY_TYPE.off, configsLinkOrWord, configLinkOrWord),
        ]
            .filter(Boolean)
            .join(' ');
    ...
    [NOTICE_TYPE.DEPRECATED]: ({ context, deprecated, replacedBy, ruleName }) => {
        ...
        const pathCurrentPage = join(getPluginRoot(path), replaceRulePlaceholder(pathRuleDoc, ruleName));
        // New DeprecatedInfo object format (ESLint >=9.21.0) vs legacy boolean format.
        if (typeof deprecated === 'object') {
            function formatVersion(version) {
                return version.startsWith('v') ? version : `v${version}`;
            }
            const sentenceDeprecated = `${EMOJI_DEPRECATED} This rule ${deprecated.deprecatedSince ? 'has been' : 'is'} ${deprecated.url ? `[deprecated](${deprecated.url})` : 'deprecated'}${deprecated.deprecatedSince
                ...
                : ''}.`;
            const sentenceReplacedBy = replacedByToNoticeSentence(context, deprecated, pathCurrentPage);
        ...
        const replacementRuleList = (replacedBy ?? []).map((replacementRuleName) => {
            return getLinkToRule(context, replacementRuleName, pathCurrentPage, true, true);
        });
 ...
 */
function getRuleNoticeLines(context, ruleName) {
    ...
    const configsError = getConfigsForRule(context, ruleName, SEVERITY_TYPE.error).filter((configName) => !ignoreConfig.includes(configName));
    const configsWarn = getConfigsForRule(context, ruleName, SEVERITY_TYPE.warn).filter((configName) => !ignoreConfig.includes(configName));
    const configsOff = getConfigsForRule(context, ruleName, SEVERITY_TYPE.off).filter((configName) => !ignoreConfig.includes(configName));
    const notices = getNoticesForRule(context, rule, configsError, configsWarn, configsOff);
    let noticeType;
 ...
 */
export function generateRuleHeaderLines(context, description, name) {
